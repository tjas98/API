...
var noviUrnik = (req, res) => {
    ...
    for (var i = 2; i < urnik2.length; i++) {
        ...
        else razred = razred.toUpperCase()
    ...
    function preglejCe(razred) {
        if (razred.includes(",") || razred.includes("-")) return 2
        else if (razred.includes(" ")) return 1
        else return 0
    }

    
    function uredi(profesor, razred, tip, dan) {
        
        //console.log(dan, profesor, razred)
        if (tip == 0) x = razred
        if (tip == 1) x = razred.split(" ")[0]
        if (tip == 2) {
            x = razred.split(",")
            x = razred.split("-")
            for (var i = 0; i < x.length; i++) {
                x[i] = x[i].replace(/ /g,'')
                var p = urnikRazrediProfesorji[x[i]][dan][j].length  
                if (p == 0)  urnikRazrediProfesorji[x[i]][dan][j] = profesor;
                else urnikRazrediProfesorji[x[i]][dan][j] += "," + profesor;
            }
        } else {
            //console.log(urnikRazrediProfesorji[x][dan])
            console.log(dan, profesor, razred)
            if (urnikRazrediProfesorji[x][dan] != undefined) {
                
                var p = urnikRazrediProfesorji[x][dan][j].length  
                if (p == 0)  urnikRazrediProfesorji[x][dan][j] = profesor;
                else urnikRazrediProfesorji[x][dan][j] += "," + profesor;
            }
    ...
    for (i in profesorji) {
        ...
        for (var j = 0; j < 7; j++) {
            var x = urnik['pon'][j].toUpperCase()
            if (razredi.some(a=>x.includes(a))) {
                var a = preglejCe(x)
                uredi(profesor, x, a, "pon")
            }

            var x = urnik['tor'][j].toUpperCase()
            if (razredi.some(a=>x.includes(a))) {
                var a = preglejCe(x)
                uredi(profesor, x, a, "tor")
            }

            var x = urnik['sre'][j].toUpperCase()
            if (razredi.some(a=>x.includes(a))) {
                var a = preglejCe(x)
                uredi(profesor, x, a, "sre")
            }

            var x = urnik['cet'][j].toUpperCase()
            if (razredi.some(a=>x.includes(a))) {
                var a = preglejCe(x)
                uredi(profesor, x, a, "cet")
            }

            var x = urnik['pet'][j].toUpperCase()
            if (razredi.some(a=>x.includes(a))) {
                var a = preglejCe(x)
                uredi(profesor, x, a, "pet")
            }
